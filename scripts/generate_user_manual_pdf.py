...
DEFAULT_OUTPUT = PROJECT_ROOT / "docs" / "WBPriceAnalyzer_Instruction_v0.2.11.pdf"
VERSION = "0.2.11"
...
def _later_page(canvas, doc) -> None:
    ...
    canvas.setFont(FONT, 7.5)
    canvas.drawString(18 * mm, 8.5 * mm, "otdelvsego-spec/WBPriceAnalyzer3")
...
def build_story() -> list[object]:
    ...
    story.extend(
        [
            ...
            Spacer(1, 13 * mm),
            callout(
                "Главное в версии 0.2.11",
                "Инструкция добавлена в Windows ZIP. Описаны старый и новый форматы WB, "
                "неполные периоды и единый выбор отчётов для вкладок «Обзор», «Исходные файлы», "
                "«Разбивка» и «Сценарий цены».",
            ),
            Spacer(1, 27 * mm),
        ...
        [
            ...
            data_table(
                ...
                [
                    ...
                    ["Новый артикул", "Нужно создать его в справочнике или явно пропустить."],
                ],
                [62, 104],
            ),
            p("После импорта", "h2"),
    ...
    story.extend(
        [
            ...
            data_table(
                ["Ситуация", "Что делать"],
                [
                    ["Программа не видит обязательный столбец", "Убедитесь, что загружен исходный XLSX, а не PDF и не изменённая копия. Версия 0.2.11 поддерживает старые и новые заголовки WB."],
            ...
            callout(
                ...
                "Исходный код и Windows-сборки: https://github.com/otdelvsego-spec/WBPriceAnalyzer3",
                color=SUCCESS,
            ),
            Spacer(1, 8 * mm),
