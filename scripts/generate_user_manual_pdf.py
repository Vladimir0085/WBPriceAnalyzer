...
DEFAULT_OUTPUT = PROJECT_ROOT / "docs" / "WBPriceAnalyzer_Instruction_v0.2.16.pdf"
VERSION = "0.2.16"
...
def build_story() -> list[object]:
    ...
    story.extend(
        [
            ...
            Spacer(1, 13 * mm),
            callout(
                "Главное в версии 0.2.16",
                "Интерфейс приведён к OZ Price Analyzer 0.5.31: строки «Сценария цены» "
                "переносятся в узком окне, список «Отчёт» показывает «№4 · 14.09–20.09.2026» "
                "с полным названием в подсказке, на каждой карточке «Обзора» — подсказка с "
                "формулой из «Справки», карточка «Баллы» называется «Лояльность и баллы (расход)».",
            ),
            Spacer(1, 27 * mm),
            plain("Актуально на 26.09.2026", "small"),
            plain("Расчёты, налог (Выручка MAIN + BUYOUT) и экспорт не изменены.", "small"),
            PageBreak(),
        ]
    )
        ...
        [
            ...
            numbered_list(
                [
                    ...
                    "Откройте «Справочник себестоимости» и загрузите справочник XLSX, затем укажите налоговую ставку в «Настройках».",
                    "Нажмите «Импортировать отчёты» и выберите все связанные файлы одного расчёта одновременно.",
                    "Проверьте итоги в «Обзоре», а затем сохраните результат через «Экспорт в Excel».",
                ]
            ),
            callout(
                "Где хранятся данные",
            ...
            p("Масштаб и шапка окна", "h2"),
            p("В «Настройках» можно выбрать масштаб «Авто», 100%, 90% или 80%. При 100% строки таблиц имеют высоту 30 px, "
              "при меньшем масштабе уменьшаются строки, отступы кнопок и вкладок, а подзаголовок шапки скрывается. "
              "На экранах до 1440x900 окно открывается развёрнутым. Кнопки «Импортировать отчёты», «Экспорт в Excel» "
              "и «О программе» расположены в одну строку и в узком окне переносятся на следующую строку."),
            p("Список «Отчёт» показывает отчёт кратко: «№4 · 14.09–20.09.2026»; однодневный отчёт — «№4 · 31.08.2026». "
              "Отчёт, переименованный вручную на вкладке «История отчётов», показан своим именем. Полное название "
              "и период видны во всплывающей подсказке при наведении на список."),
            PageBreak(),
        ]
    )
    ...
    story.extend(
        [
            ...
            callout(
                ...
                "УПД, УКД, счета-фактуры, акты, PDF, XML, банковские выписки и скриншоты не входят в расчёт. Справочник себестоимости загружается отдельно во вкладке «Справочник себестоимости».",
            ),
            PageBreak(),
        ]
    )
    ...
    story.extend(
        [
            ...
            p("В верхнем списке отчёт подписан кратко, например «№4 · 14.09–20.09.2026» или своим именем после "
              "переименования; полное название — во всплывающей подсказке."),
            Spacer(1, 5 * mm),
            data_table(
            ...
            data_table(
                ...
                [
                    ...
                    ["Сценарий цены", "Сводные текущие и плановые показатели и категории всех выбранных отчётов. Изменение плановых цен блокируется, пока выбрано более одного отчёта."],
                ],
                [45, 121],
            ),
            p("Пример", "h2"),
    ...
    story.extend(
        [
            ...
            p("Подсказка на каждой из 17 карточек «Обзора» показывает её формулу из «Справки». Карточка «Комиссия WB» "
              "справочная: комиссия уже учтена в сумме «К перечислению продавцу». Карточка «Лояльность и баллы (расход)» "
              "показывает расход программы лояльности WB: стоимость участия и удержанные баллы."),
            p("Нераспределённые операции", "h2"),
            formula("Чистая прибыль от деятельности = Чистая прибыль товаров + Нераспределённые"),
            Spacer(1, 2 * mm),
            formula("Доходность деятельности = Чистая прибыль от деятельности / С/с проданного x 100%"),
        ...
        [
            ...
            data_table(
                ...
                [
                    ...
                    ["Чистая прибыль товаров", "Прибыль товаров без общих операций без артикула."],
                    ...
                    ["Чистая прибыль от деятельности", "Прибыль отчёта с нераспределёнными и её доходность; налог с нераспределённых не начисляется."],
                ],
                [57, 109],
            ),
            ...
            Spacer(1, 1.5 * mm),
            formula("Лояльность и баллы (расход), % = Лояльность и баллы / Общая выручка x 100%"),
            Spacer(1, 1.5 * mm),
            formula("Чистая прибыль, % от выручки = Чистая прибыль от деятельности / Общая выручка x 100%"),
            p("Итоги по фильтру категорий", "h2"),
            p("Кнопка категории открывает список, где можно отметить одну или несколько категорий. Пустой выбор означает «Все категории». "
              "«Исключить выбранные» оставляет остальные категории; без отмеченных категорий показываются все товары. "
              "Блок «Итоги по фильтру» суммирует только товары фильтра, например «Итоги по фильтру: категории: Горшки · 2 позиции». "
              "Нераспределённые операции категории не имеют и в этот блок не входят."),
            # Keep the heading with its list so it never stays alone at a page bottom.
            KeepTogether(
                [
                    ...
                    bullet_list(
                        [
                            ...
                            "Финрезультат WB на ед. = финрезультат товара / количество продаж; «Финрезультат WB до с/с и налога» - финрезультат товара целиком.",
        ...
        [
            ...
            data_table(
                ...
                [
                    ...
                    ["Настройки", "Налоговая ставка, тема, масштаб интерфейса, пересчёт истории, резервные копии и папка данных."],
                ],
                [51, 115],
            ),
            p("Сценарий цены", "h2"),
            bullet_list(
                [
                    ...
                    "Карточки «Текущая выручка», «Плановая выручка», «Плановая чистая прибыль» и «Плановая доходность» находятся под таблицей и учитывают фильтр категорий.",
                ]
            ),
            p("Фильтры и размер таблиц", "h2"),
            p("Категория, артикул, метрика сортировки и направление применяются к таблице. "
              "Фильтры «Обзора» и «Сценария цены» и строка «Плановая цена…» переносятся на новые строки при нехватке ширины. "
              "Сводка и таблица делят поровну место, оставшееся после панели фильтров. "
              "Если сводка не помещается, используйте её прокрутку. «На весь экран» скрывает "
              "сводку, сохраняя фильтры; кнопки меняются на «Вернуть обычный вид» и «Открыть отдельно». "
              "«↗ Отдельно» открывает синхронизированную таблицу в новом окне. F11 разворачивает таблицу, "
        ...
        [
            ...
            bullet_list(
                [
                    ...
                    "Строки без полной с/с не участвуют в расчёте, пока значение не будет заполнено; кнопка «Показать пропущенные строки» появляется только при таких строках.",
                    "«Удалить» убирает позицию только из текущего справочника: сохранённые отчёты и историческая с/с не меняются.",
                    "Изменение каталога не переписывает историческую с/с без явного пересчёта истории.",
                ]
            ),
            p("Экспорт в XLSX", "h2"),
            p("Экспорт создаёт книгу с листами «Итог», «Разбивка» и «Справочник операций». Процентные метрики, чистая прибыль от деятельности, её доходность и нераспределённые операции переносятся в файл (строка «Итого от деятельности (с нераспределенными)»)."),
            callout(
