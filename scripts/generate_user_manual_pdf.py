...
DEFAULT_OUTPUT = PROJECT_ROOT / "docs" / "WBPriceAnalyzer_Instruction_v0.2.13.pdf"
VERSION = "0.2.13"
...
def build_story() -> list[object]:
    ...
    story.extend(
        [
            ...
            Spacer(1, 13 * mm),
            callout(
                "Главное в версии 0.2.13",
                "Карточки показателей на вкладке «Обзор» стали компактнее. "
                "Товарная таблица автоматически получает половину рабочей области "
                "вкладки при любом размере окна.",
            ),
            Spacer(1, 27 * mm),
    ...
    story.extend(
        [
            ...
            p("Категория, артикул, метрика сортировки и направление применяются к таблице. "
              "На вкладке «Обзор» верхние показатели и товарная таблица автоматически "
              "делят рабочую область поровну. Кнопка «На весь экран» временно скрывает "
              "верхние блоки, а «Отдельно» открывает синхронизированную таблицу в новом окне."),
            PageBreak(),
        ]
    )
